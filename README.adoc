...
`jenkins.task.queuing` (timer):: The rate at which tasks are queued and the total time they spend in the build queue.
`jenkins.task.waiting` (timer):: The rate at which tasks enter the quiet period and the total amount of time that jobs spend in their quiet period.
`jenkins.task.blocked` (timer):: The rate at which tasks in the build queue enter the blocked state and the amount of time they spend in that state.
`jenkins.task.buildable` (timer):: The rate at which tasks in the build queue enter the buildable state and the amount of time they spend in that state.
`jenkins.task.execution` (timer):: The rate at which tasks in the build queue enter the execution state and the amount of time they spend in that state.
