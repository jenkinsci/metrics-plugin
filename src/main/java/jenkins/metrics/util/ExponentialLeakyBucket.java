 ...
 */
public class ExponentialLeakyBucket<T> {
    private static final int DEFAULT_SIZE = 1028;
    private static final double DEFAULT_ALPHA = 0.015;
    private static final long RESCALE_THRESHOLD = TimeUnit.HOURS.toNanos(1);

    private final ConcurrentSkipListMap<Double, T> values;
    ...
    /**
     * Creates a new {@link ExponentialLeakyBucket} of 1028 elements, which offers a 99.9%
    ...
    /**
     * Creates a new {@link ExponentialLeakyBucket}.
     *
     ...
     */
    public ExponentialLeakyBucket(int size, double alpha, Clock clock) {
        this.values = new ConcurrentSkipListMap<Double, T>();
    ...
    public void add(T value) {
        add(value, currentTimeInSeconds());
    }
    ...
    private void rescale(long now, long next) {
        if (nextScaleTime.compareAndSet(next, now + RESCALE_THRESHOLD)) {
            ...
            try {
                ...
                for (Double key : keys) {
                    final T value = values.remove(key);
